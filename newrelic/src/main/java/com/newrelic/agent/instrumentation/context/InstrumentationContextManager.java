...
public class InstrumentationContextManager
{
    ...
    public InstrumentationContextManager(final Instrumentation instrumentation) {
        this.matchVisitors = (Map<ClassMatchVisitorFactory, ContextClassTransformer>)Maps.newConcurrentMap();
        this.interfaceMatchVisitors = (Map<ClassMatchVisitorFactory, ContextClassTransformer>)Maps.newConcurrentMap();
        this.FinishClassTransformer = new ContextClassTransformer() {
            ...
            private byte[] getFinalTransformation(final ClassLoader loader, final String className, final Class<?> classBeingRedefined, final byte[] classfileBuffer, final InstrumentationContext context) {
                final ClassReader reader = new ClassReader(classfileBuffer);
                ClassVisitor cv;
                final ClassWriter writer = (ClassWriter)(cv = new PatchedClassWriter(2, context.getClassResolver(loader)));
                ...
                cv = new ClassVisitor(327680, cv) {
                    public void visit(int version, final int access, final String name, final String signature, final String superName, final String[] interfaces) {
                        if (version < 49 || version > 100) {
                            Agent.LOG.log(Level.FINEST, "Converting {0} from version {1} to {2}", new Object[] { name, version, 49 });
                            version = 49;
                        }
            ...
            private void addSupportabilityMetrics(final ClassReader reader, final String className, final InstrumentationContext context) {
        ...
        if (agentConfig.getValue("instrumentation.web_services.enabled", (Object)true)) {
            this.matchVisitors.put(new WebServiceVisitor(), InstrumentationContextManager.NO_OP_TRANSFORMER);
        }
        if (agentConfig.getValue("instrumentation.rest_annotations.enabled", (Object)true)) {
        ...
        if (agentConfig.getValue("instrumentation.spring_annotations.enabled", (Object)true)) {
        ...
        if (agentConfig.getValue("instrumentation.servlet_annotations.enabled", (Object)true)) {
            this.matchVisitors.put(new ServletAnnotationVisitor(), InstrumentationContextManager.NO_OP_TRANSFORMER);
        }
        final Config instrumentationConfig = agentConfig.getClassTransformerConfig().getInstrumentationConfig("com.newrelic.instrumentation.ejb-3.0");
        ...
        this.matchVisitors.put(ServiceFactory.getJarCollectorService().getSourceVisitor(), InstrumentationContextManager.NO_OP_TRANSFORMER);
        try {
    ...
    public static InstrumentationContextManager create(final InstrumentationProxy instrumentation, final boolean bootstrapClassloaderEnabled) throws Exception {
        ...
        final ClassFileTransformer transformer = new ClassFileTransformer() {
            public byte[] transform(ClassLoader loader, final String className, final Class<?> classBeingRedefined, final ProtectionDomain protectionDomain, byte[] classfileBuffer) throws IllegalClassFormatException {
                if (className.startsWith("com/newrelic/agent/deps/org/objectweb/asm") || className.startsWith("com/newrelic/deps") || className.startsWith("com/newrelic/agent/tracers/")) {
                    return null;
                }
                ...
                final InstrumentationContext context = new InstrumentationContext(classfileBuffer, classBeingRedefined, protectionDomain);
                ...
                for (final Map.Entry<ClassMatchVisitorFactory, OptimizedClassMatcher.Match> entry : context.getMatches().entrySet()) {
                    ...
                    if (transformer != null && transformer != InstrumentationContextManager.NO_OP_TRANSFORMER) {
                        final byte[] bytes = transformer.transform(loader, className, classBeingRedefined, protectionDomain, classfileBuffer, context, entry.getValue());
                ...
                if (context.isTracerMatch()) {
                    final byte[] bytes2 = traceTransformer.transform(loader, className, classBeingRedefined, protectionDomain, classfileBuffer, context, null);
    ...
    private void applyInterfaceVisitors(final ClassLoader loader, final Class<?> classBeingRedefined, final ClassReader reader) {
    ...
    public void addContextClassTransformer(final ClassMatchVisitorFactory matchVisitor, ContextClassTransformer transformer) {
        if (transformer == null) {
        ...
        return new ClassVisitor(327680, cv) {
            ...
            public MethodVisitor visitMethod(final int access, final String name, final String desc, final String signature, final String[] exceptions) {
                ...
                if (context.isModified(method) && loader != null) {
                    ...
                    final List<String> instrumentationNames = (List<String>)Lists.newArrayList();
                    final List<InstrumentationType> instrumentationTypes = (List<InstrumentationType>)Lists.newArrayList();
    ...
    static {
        MARKER_INTERFACES_TO_SKIP = ImmutableSet.of("org/hibernate/proxy/HibernateProxy", "org/springframework/aop/SpringProxy");
        NO_OP_TRANSFORMER = new ContextClassTransformer() {
            public byte[] transform(final ClassLoader loader, final String className, final Class<?> classBeingRedefined, final ProtectionDomain protectionDomain, final byte[] classfileBuffer, final InstrumentationContext context, final OptimizedClassMatcher.Match match) throws IllegalClassFormatException {
                return null;
            }
        };
        ANNOTATIONS_TO_REMOVE = ImmutableSet.of(Type.getDescriptor(InstrumentedClass.class), Type.getDescriptor(InstrumentedMethod.class), Type.getDescriptor(WeavedMethod.class));
    }
    ...
    private static class MarkWeaverMethodsVisitor extends ClassVisitor
    {
        ...
        public MethodVisitor visitMethod(final int access, final String name, final String desc, final String signature, final String[] exceptions) {
            final MethodVisitor mv = super.visitMethod(access, name, desc, signature, exceptions);
            final Collection<String> instrumentationTitles = this.context.getMergeInstrumentationPackages(new Method(name, desc));
            if (instrumentationTitles != null && !instrumentationTitles.isEmpty()) {
                final AnnotationVisitor weavedAnnotation = mv.visitAnnotation(Type.getDescriptor(WeavedMethod.class), true);
