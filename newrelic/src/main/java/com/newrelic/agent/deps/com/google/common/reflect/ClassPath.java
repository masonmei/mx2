...
import com.newrelic.agent.deps.com.google.common.collect.ImmutableSortedSet;
...
public final class ClassPath
{
    ...
    public ImmutableSet<ClassInfo> getTopLevelClassesRecursive(final String packageName) {
        Preconditions.checkNotNull(packageName);
        final String value = String.valueOf(String.valueOf(packageName));
        final String packagePrefix = new StringBuilder(1 + value.length()).append(value).append(".").toString();
        final ImmutableSet.Builder<ClassInfo> builder = ImmutableSet.builder();
        for (final ClassInfo classInfo : this.getTopLevelClasses()) {
    ...
    static final class Scanner
    {
        ...
        private void scanDirectory(final File directory, final ClassLoader classloader, final String packagePrefix, final ImmutableSet<File> ancestors) throws IOException {
            ...
            if (files == null) {
                ...
                final String value = String.valueOf(String.valueOf(directory));
                access$100.warning(new StringBuilder(22 + value.length()).append("Cannot read directory ").append(value).toString());
                return;
            }
            ...
            for (final File f : files) {
                ...
                if (f.isDirectory()) {
                    final File directory2 = f;
                    final String value2 = String.valueOf(String.valueOf(packagePrefix));
                    final String value3 = String.valueOf(String.valueOf(name));
                    this.scanDirectory(directory2, classloader, new StringBuilder(1 + value2.length() + value3.length()).append(value2).append(value3).append("/").toString(), newAncestors);
                }
                else {
                    final String value4 = String.valueOf(packagePrefix);
                    final String value5 = String.valueOf(name);
                    final String resourceName = (value5.length() != 0) ? value4.concat(value5) : new String(value4);
            ...
            if (classpathAttribute != null) {
                for (final String path : ClassPath.CLASS_PATH_ATTRIBUTE_SEPARATOR.split(classpathAttribute)) {
                    ...
                    try {
                        ...
                        final Logger access$100 = ClassPath.logger;
                        final String s = "Invalid Class-Path entry: ";
                        final String value = String.valueOf(path);
                        access$100.warning((value.length() != 0) ? s.concat(value) : new String(s));
                        continue;
                    }
                    builder.add(uri);
                }
