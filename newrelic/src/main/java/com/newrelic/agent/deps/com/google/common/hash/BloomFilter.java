...
import com.newrelic.agent.deps.com.google.common.primitives.UnsignedBytes;
import com.newrelic.agent.deps.com.google.common.primitives.SignedBytes;
...
public final class BloomFilter<T> implements Predicate<T>, Serializable
{
    ...
    @VisibleForTesting
    static <T> BloomFilter<T> create(final Funnel<? super T> funnel, int expectedInsertions, final double fpp, final Strategy strategy) {
    ...
    public static <T> BloomFilter<T> readFrom(final InputStream in, final Funnel<T> funnel) throws IOException {
        ...
        try {
            ...
            final IOException ioException = new IOException(new StringBuilder(65 + value.length()).append(value).append(strategyOrdinal).append(" numHashFunctions: ").append(numHashFunctions).append(" dataLength: ").append(dataLength).toString());
    ...
    {
         <T> boolean put(T p0, Funnel<? super T> p1, int p2, BloomFilterStrategies.BitArray p3);
        
         <T> boolean mightContain(T p0, Funnel<? super T> p1, int p2, BloomFilterStrategies.BitArray p3);
