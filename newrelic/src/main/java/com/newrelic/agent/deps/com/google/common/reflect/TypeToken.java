...
import com.newrelic.agent.deps.com.google.common.collect.ForwardingSet;
...
import com.newrelic.agent.deps.com.google.common.primitives.Primitives;
...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable
{
    ...
    final TypeToken<T> rejectTypeVariables() {
        new TypeVisitor() {
            ...
            void visitTypeVariable(final TypeVariable<?> type) {
                ...
                throw new IllegalArgumentException(new StringBuilder(58 + value.length()).append(value).append("contains a type variable and is not safe for the operation").toString());
            }
    ...
    private static Type supertypeBound(final WildcardType type) {
        ...
        final String value = String.valueOf(String.valueOf(type));
        throw new AssertionError((Object)new StringBuilder(59 + value.length()).append("There should be at most one upper bound for wildcard type: ").append(value).toString());
    }
    ...
    private static Type subtypeBound(final WildcardType type) {
        ...
        final String value = String.valueOf(String.valueOf(type));
        throw new AssertionError((Object)new StringBuilder(46 + value.length()).append("Wildcard should have at most one lower bound: ").append(value).toString());
    }
    ...
    private TypeToken<? super T> getSupertypeFromUpperBounds(final Class<? super T> supertype, final Type[] upperBounds) {
        ...
        final String value = String.valueOf(String.valueOf(supertype));
        final String value2 = String.valueOf(String.valueOf(this));
        throw new IllegalArgumentException(new StringBuilder(23 + value.length() + value2.length()).append(value).append(" isn't a super type of ").append(value2).toString());
    }
    
    private TypeToken<? extends T> getSubtypeFromLowerBounds(final Class<?> subclass, final Type[] lowerBounds) {
        final int len$ = lowerBounds.length;
        final int i$ = 0;
        if (i$ < len$) {
            final Type lowerBound = lowerBounds[i$];
            final TypeToken<? extends T> bound = (TypeToken<? extends T>)of(lowerBound);
            return bound.getSubtype(subclass);
        }
        final String value = String.valueOf(String.valueOf(subclass));
        final String value2 = String.valueOf(String.valueOf(this));
        throw new IllegalArgumentException(new StringBuilder(21 + value.length() + value2.length()).append(value).append(" isn't a subclass of ").append(value2).toString());
    }
