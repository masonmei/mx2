...
public final class Iterators
{
    ...
    public static <T> T getOnlyElement(final Iterator<T> iterator) {
        ...
        final String value = String.valueOf(String.valueOf(first));
        sb2.append(new StringBuilder(31 + value.length()).append("expected one element but was: <").append(value).toString());
        for (int i = 0; i < 4 && iterator.hasNext(); ++i) {
            final StringBuilder sb3 = sb;
            final String value2 = String.valueOf(String.valueOf(iterator.next()));
            sb3.append(new StringBuilder(2 + value2.length()).append(", ").append(value2).toString());
        }
        if (iterator.hasNext()) {
            sb.append(", ...");
        }
