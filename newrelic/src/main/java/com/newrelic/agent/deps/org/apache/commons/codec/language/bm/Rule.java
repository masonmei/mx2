...
public class Rule
{
    ...
    private static String createResourceName(final NameType nameType, final RuleType rt, final String lang) {
        return String.format("com/newrelic/agent/deps/org/apache/commons/codec/language/bm/%s_%s_%s.txt", nameType.getName(), rt.getName(), lang);
    }
    ...
    private static Scanner createScanner(final String lang) {
        final String resName = String.format("com/newrelic/agent/deps/org/apache/commons/codec/language/bm/%s.txt", lang);
        final InputStream rulesIS = Languages.class.getClassLoader().getResourceAsStream(resName);
        if (rulesIS == null) {
            throw new IllegalArgumentException("Unable to load resource: " + resName);
        }
    ...
    public static List<Rule> getInstance(final NameType nameType, final RuleType rt, final String lang) {
        final List<Rule> rules = Rule.RULES.get(nameType).get(rt).get(lang);
        if (rules == null) {
    ...
    public boolean patternAndContextMatches(final CharSequence input, final int i) {
        ...
        final boolean patternMatches = input.subSequence(i, ipl).equals(this.pattern);
        final boolean rContextMatches = this.rContext.isMatch(input.subSequence(ipl, input.length()));
        final boolean lContextMatches = this.lContext.isMatch(input.subSequence(0, i));
    ...
    static {
        ...
        RULES = new EnumMap<NameType, Map<RuleType, Map<String, List<Rule>>>>(NameType.class);
        for (final NameType s : NameType.values()) {
            ...
            for (final RuleType rt : RuleType.values()) {
                ...
                rts.put(rt, Collections.unmodifiableMap((Map<? extends String, ? extends List<Rule>>)rs));
            }
            Rule.RULES.put(s, Collections.unmodifiableMap((Map<? extends RuleType, ? extends Map<String, List<Rule>>>)rts));
        }
    }
    ...
    {
        boolean isMatch(CharSequence p0);
    }
