...
import com.newrelic.agent.deps.javassist.CtNewConstructor;
...
public class StubGenerator implements Translator
{
    ...
    private static final String sampleClass = "com.newrelic.agent.deps.javassist.tools.rmi.Sample";
    ...
    public void start(final ClassPool pool) throws NotFoundException {
        this.classPool = pool;
        final CtClass c = pool.get("com.newrelic.agent.deps.javassist.tools.rmi.Sample");
        this.forwardMethod = c.getDeclaredMethod("forward");
        this.forwardStaticMethod = c.getDeclaredMethod("forwardStatic");
        this.proxyConstructorParamTypes = pool.get(new String[] { "com.newrelic.agent.deps.javassist.tools.rmi.ObjectImporter", "int" });
        this.interfacesForProxy = pool.get(new String[] { "java.io.Serializable", "com.newrelic.agent.deps.javassist.tools.rmi.Proxy" });
        this.exceptionForProxy = new CtClass[] { pool.get("com.newrelic.agent.deps.javassist.tools.rmi.RemoteException") };
    }
    ...
    private CtClass produceProxyClass(final CtClass orgclass, final Class orgRtClass) throws CannotCompileException, NotFoundException {
        ...
        CtField f = new CtField(this.classPool.get("com.newrelic.agent.deps.javassist.tools.rmi.ObjectImporter"), "importer", proxy);
        f.setModifiers(2);
