...
public abstract class Utils
{
    ...
    public static Member getMemberFromDescriptor(final String descriptor, final ClassLoader... classLoaders) throws ReflectionsException {
        ...
        final int p = Math.max(memberKey.lastIndexOf(46), memberKey.lastIndexOf("$"));
        final String className = memberKey.substring(memberKey.lastIndexOf(32) + 1, p);
        final String memberName = memberKey.substring(p + 1);
    ...
    public static Logger findLogger(final Class<?> aClass) {
        try {
            Class.forName("com.newrelic.agent.deps.org.slf4j.impl.StaticLoggerBinder");
