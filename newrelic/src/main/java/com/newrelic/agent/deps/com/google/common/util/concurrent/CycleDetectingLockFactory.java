...
public class CycleDetectingLockFactory
{
    ...
    private static String getLockName(final Enum<?> rank) {
        final String value = String.valueOf(String.valueOf(rank.getDeclaringClass().getSimpleName()));
        final String value2 = String.valueOf(String.valueOf(rank.name()));
    ...
    private static class ExampleStackTrace extends IllegalStateException
    {
        ...
        ExampleStackTrace(final LockGraphNode node1, final LockGraphNode node2) {
            final String value = String.valueOf(String.valueOf(node1.getLockName()));
            final String value2 = String.valueOf(String.valueOf(node2.getLockName()));
            super(new StringBuilder(4 + value.length() + value2.length()).append(value).append(" -> ").append(value2).toString());
    ...
    private static class LockGraphNode
    {
        ...
        void checkAcquiredLock(final Policy policy, final LockGraphNode acquiredLock) {
            final boolean expression = this != acquiredLock;
            final String s = "Attempted to acquire multiple locks with the same rank ";
            final String value = String.valueOf(acquiredLock.getLockName());
            Preconditions.checkState(expression, (Object)((value.length() != 0) ? s.concat(value) : new String(s)));
    ...
    {
        void handlePotentialDeadlock(PotentialDeadlockException p0);
    }
}
