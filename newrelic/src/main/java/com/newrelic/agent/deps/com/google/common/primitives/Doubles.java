...
package com.newrelic.agent.deps.com.google.common.primitives;
...
public final class Doubles
{
    ...
    private static Pattern fpPattern() {
        ...
        final String completeDec = String.valueOf(decimal).concat("(?:[eE][+-]?\\d++)?[fFdD]?");
        final String hex = "(?:\\p{XDigit}++(?:\\.\\p{XDigit}*+)?|\\.\\p{XDigit}++)";
        final String value = String.valueOf(String.valueOf(hex));
        final String completeHex = new StringBuilder(25 + value.length()).append("0[xX]").append(value).append("[pP][+-]?\\d++[fFdD]?").toString();
        final String value2 = String.valueOf(String.valueOf(completeDec));
        final String value3 = String.valueOf(String.valueOf(completeHex));
        final String fpPattern = new StringBuilder(23 + value2.length() + value3.length()).append("[+-]?(?:NaN|Infinity|").append(value2).append("|").append(value3).append(")").toString();
    ...
    private enum LexicographicalComparator implements Comparator<double[]>
    {
        ...
        public int compare(final double[] left, final double[] right) {
            for (int minLength = Math.min(left.length, right.length), i = 0; i < minLength; ++i) {
                final int result = Double.compare(left[i], right[i]);
                if (result != 0) {
                    return result;
                }
            }
