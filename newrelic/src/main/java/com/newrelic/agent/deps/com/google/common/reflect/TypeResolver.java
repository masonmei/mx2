...
public final class TypeResolver
{
    ...
    private static void populateTypeMappings(final Map<TypeVariableKey, Type> mappings, final Type from, final Type to) {
        ...
        new TypeVisitor() {
            ...
            void visitClass(final Class<?> fromClass) {
                final String value = String.valueOf(String.valueOf(fromClass));
                throw new IllegalArgumentException(new StringBuilder(21 + value.length()).append("No type mapping from ").append(value).toString());
            }
        }.visit(from);
    }
    ...
    private static <T> T expectArgument(final Class<T> type, final Object arg) {
        try {
            ...
            final String value = String.valueOf(String.valueOf(arg));
            final String value2 = String.valueOf(String.valueOf(type.getSimpleName()));
            throw new IllegalArgumentException(new StringBuilder(10 + value.length() + value2.length()).append(value).append(" is not a ").append(value2).toString());
        }
    }
        ...
        final Type resolve(final TypeVariable<?> var) {
            final TypeTable guarded = new TypeTable() {
                public Type resolveInternal(final TypeVariable<?> intermediateVar, final TypeTable forDependent) {
                    ...
                    return TypeTable.this.resolveInternal(intermediateVar, forDependent);
                }
            };
    ...
    private static final class WildcardCapturer
    {
        ...
        Type capture(final Type type) {
            ...
            if (lowerBounds.length == 0) {
                ...
                final String name = new StringBuilder(33 + value.length()).append("capture#").append(incrementAndGet).append("-of ? extends ").append(value).toString();
