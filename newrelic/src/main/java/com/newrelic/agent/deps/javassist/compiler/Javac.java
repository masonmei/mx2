...
import com.newrelic.agent.deps.javassist.CtMember;
...
public class Javac
{
    ...
    public void recordSpecialProceed(final String target, final String classname, final String methodname, final String descriptor) throws CompileError {
        final Parser p = new Parser(new Lex(target));
        final ASTree texpr = p.parseExpression(this.stable);
        final ProceedHandler h = new ProceedHandler() {
            @Override
